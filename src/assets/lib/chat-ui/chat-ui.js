	...
	var ChatUI = function ChatUI(chatData) {
	    ...
	    return {
	        render: function render(querySelector, componentName) {
				var baseEl;
				if(typeof querySelector.nodeType === 'number'){
					baseEl = querySelector;
				}else if(typeof querySelector === 'string') {
					baseEl = document.querySelector(querySelector);
				}
	            if (!baseEl) {
	...
	var Dialog = exports.Dialog = function (_Component) {
	    ...
	    _createClass(Dialog, [{
	        ...
	        value: function scrollDialogDown() {
	            this.refs.dialogEl.scrollTop = this.refs.dialogEl.scrollHeight - document.querySelectorAll('.chat-ui-dialog-bubble-container:last-child')[0].offsetHeight - 200;
	        }
	    }, {
	        key: 'addEvents',
	        value: function addEvents() {
